import React from 'react'
import Image from "next/image";

function Banner() {

    
    return (
        <div className="relative h-[300px] sm:h-[400px] lg:h-[500px] xl:h-[600px] 2xl:[700px] ">
            <Image src="https://links.papareact.com/0fm"
            layout="fill"
            objectFit="cover"
            />
            <div className="absolute top-1/2 w-full text-center">
                <p className="text-xl font-bold sm:text-sm">Not Sure where to go? Perfect.</p>
                <button className="font-bold text-purple-500
                 bg-white px-10 py-4
                  shadow-md rounded-full my-3 hover:shadow-xl active:scale-90 transition duration-100">
                    I'm Flexible
                </button>
            </div>
        </div>
    )
}

export default Banner
